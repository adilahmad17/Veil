...
use crate::mem::pgtable_print_pte_va;
use crate::service::*;
...
/// 8
const SVSM_CREATE_ENCLAVE: u32 = 8;
/// 9
const SVSM_RMPADJUST: u32 = 9;
/// 10
const SVSM_ENCLAVE_SECURE_COPY: u32 = 10;
/// 11
...
unsafe fn handle_delete_vcpu_request(vmsa: *mut Vmsa) {
    ...
    grant_vmpl_access(va, RMP_4K, VMPL::Vmpl3 as u8);


    pgtable_unmap_pages(va, VMSA_MAP_SIZE);
    
    if PERCPU.vmsa_for(VMPL::Vmpl3, cpu_id) == gpa {
        PERCPU.set_vmsa_for(PhysAddr::zero(), VMPL::Vmpl3, cpu_id);
        PERCPU.set_caa_for(PhysAddr::zero(), VMPL::Vmpl3, cpu_id);
    }
...
unsafe fn handle_create_vcpu_request(vmsa: *mut Vmsa) {
    ...
    prints!("[HARSH] Creating VCPU {apic_id}\n");
...
unsafe fn handle_pvalidate(vmsa: *mut Vmsa, entry: *const PvalidateEntry) -> (bool, bool) {
    ...
    if action == 0 {
        ...
        // prints!("[HARSH] Revoking VMPL access\n");
    ...
    if action != 0 {
        ...
        let ret: u32 = grant_vmpl_access(va, page_size, VMPL::Vmpl3 as u8);
        if ret != 0 {
            (*vmsa).set_rax(SVSM_ERR_PROTOCOL_BASE + ret as u64);
            return (false, flush);
        }
...
unsafe fn handle_pvalidate_request(vmsa: *mut Vmsa) {
    ...
    if gpa.align_down(PAGE_SIZE) != PERCPU.caa(VMPL::Vmpl3).align_down(PAGE_SIZE) {
...
unsafe fn handle_remap_ca_request(vmsa: *mut Vmsa) {
    ...
    let vmpl_disp: u8 = (*vmsa).vmpl();
    prints!("vmpl setting at {vmpl_disp}\n");
...
unsafe fn handle_create_enclave(vmsa: *mut Vmsa) {
    // TODO.
}

unsafe fn handle_rmpadjust(vmsa: *mut Vmsa) {
    // prints!("Hello from the RMPADJUST request in SVSM\n");

    let address: u64 = (*vmsa).rcx();
    let vmpl: u64 = (*vmsa).rdx();
    let num_pages: u64 = (*vmsa).r8();
    let apic_id: u32 = LOWER_32BITS!((*vmsa).r9()) as u32;
    
    // prints!("APIC ID: {apic_id}\n");
    // prints!("Address start: {:#x}\n", address);
    // prints!("VMPL requested: {vmpl}\n");
    // prints!("Number of pages: {num_pages}\n");
    
    let vmin: u64 = VMPL::Vmpl0 as u64;
    let vmax: u64 = (vmpl) as u64;

    let test_page_pa: PhysAddr = PhysAddr::new(address);
    let test_page_va: VirtAddr = match pgtable_map_pages_private(test_page_pa, 4096) {
        Ok(v) => v,
        Err(_e) => return,
    };
    // prints!("Test page map successful: {:#x}\n", test_page_va);
    pgtable_print_pte_va(test_page_va);

    // prints!("Hello, world\n");
    // let mut num = 5;
    // let r1t = &mut num as *mut i32;
    // let r1tt = *r1t;
    // prints!("r1 is: {}\n", r1tt);

    // let r1: *mut i32 = test_page_va.as_mut_ptr();
    // let r1p = *r1;
    // prints!("r1 is: {}\n", r1p);

    //TODO: Needs to be cleaned and it also needs to handle the cases where you switch to vmpl2.
    prints!("RMPADJUSTING NOW..\n");
    // let ret: u32 = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_R | vmax);
    let mut ret : u32 = 1;
    if vmax == 3 {
        prints!("RMPADJUSTING where vmax == 3\n");
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl1 as u64);
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl2 as u64);
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl3 as u64);
    }
    if vmax == 2 {
        prints!("RMPADJUSTING where vmax == 2\n");
        // ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl3 as u64);
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl1 as u64);
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl2 as u64);
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL::Vmpl3 as u64);
        // ret: u32 = rmpadjust(create_vmsa_va.as_u64(), RMP_4K, VMPL_R | VMPL_X_SUPER | VMPL::Vmpl2 as u64);
    }
    if vmax == 1 {
        prints!("RMPADJUSTING where vmax == 1\n");
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl1 as u64);
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL::Vmpl2 as u64);
        ret = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL::Vmpl3 as u64);
    }
    
    // let ret: u32 = rmpadjust(test_page_va.as_u64(), RMP_4K, vmax);
    // let ret: u32 = rmpadjust(test_page_va.as_u64(), RMP_4K, VMPL_R | VMPL_X_USER | vmax);
    if ret != 0 {
        prints!("Problem with RMPadjust: {ret}\n");
    }
    prints!("RMPADJUST complete!\n");
    // for j in 0..num_pages {
    //     prints!("VIKRAM: Starting vmin-vmax loop.\n");
    //     for i in vmin..vmax {
    //         let ret: u32 = rmpadjust(address + (j*4096), RMP_4K, VMPL_RWX | i);
    //         if ret != 0 {
    //             prints!("Problem with RMPadjust: {ret}\n");
    //         }
    //     }
    //     prints!("VIKRAM: Finished vmin->vmax loop.\n");
    //     let ret: u32 = rmpadjust(address + (j*4096), RMP_4K, VMPL::Vmpl3 as u64);
    //     if ret != 0 {
    //         prints!("Problem with RMPadjust\n");
    //     }
    // }
...
pub fn svsm_request_loop() {
    loop {
        ...
        // the request.
        //
        match map_guest_input(VMPL::Vmpl3) {
