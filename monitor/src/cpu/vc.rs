...
/// 0
const SNP_AP_CREATE_LATER: u64 = 0;
/// 1
...
/* Alternate version which doesn't immediately switch to the new VMSA state - HARSH */
pub fn enclave_vc_ap_create(vmsa_va: VirtAddr, apic_id: u32) {
    let ghcb: *mut Ghcb = vc_get_ghcb();
    let vmsa: *const Vmsa = vmsa_va.as_u64() as *const Vmsa;
   
    unsafe {
        let info1: u64 =
            GHCB_NAE_SNP_AP_CREATION_REQ!(SNP_AP_CREATE_LATER, (*vmsa).vmpl(), apic_id);
        let info2: u64 = pgtable_va_to_pa(vmsa_va).as_u64();
        (*ghcb).set_rax((*vmsa).sev_features());            
        vc_perform_vmgexit(ghcb, GHCB_NAE_SNP_AP_CREATION, info1, info2);
