...
unsafe fn __create_bios_vmsa(vmsa_va: VirtAddr) {
    //prints!("[HARSH] Inside __create_bios_vmsa\n");
    ...
    *vmsa = *bsp_page;
    ...
    (*vmsa).set_vmpl(VMPL::Vmpl3 as u8);
...
pub fn create_svsm_vmsa(for_id: usize) -> VirtAddr {
    let frame: PhysFrame = alloc_vmsa();
    let vmsa_va: VirtAddr = pgtable_pa_to_va(frame.start_address());
    let vmsa: *mut Vmsa = vmsa_va.as_mut_ptr();
...
fn ap_start(cpu_id: usize) -> bool {
    ...
    //let ret: u32 = rmpadjust(vmsa.as_u64(), RMP_4K, VMSA_PAGE | VMPL::Vmpl0 as u64);
    let ret: u32 = rmpadjust(vmsa.as_u64(), RMP_4K, VMSA_PAGE | VMPL::Vmpl3 as u64);
    ...
    unsafe {
        ...
        PERCPU.set_vmsa_for(pgtable_va_to_pa(vmsa), VMPL::Vmpl3, cpu_id);
...
/// Retrieve Vmpl3 Vmsa and start it
pub fn smp_run_bios_vmpl() -> bool {
    unsafe {
        let vmsa_pa: PhysAddr = PERCPU.vmsa(VMPL::Vmpl3);
...
pub fn smp_prepare_bios_vmpl(caa_pa: PhysAddr) -> bool {
    ...
    unsafe {
        PERCPU.set_vmsa(vmsa_pa, VMPL::Vmpl3);
        PERCPU.set_caa(caa_pa, VMPL::Vmpl3);
    }
    ...
    let ret: u32 = rmpadjust(caa.as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl3 as u64);

    if ret != 0 {
        return false;
    }

    let ret: u32 = rmpadjust(vmsa.as_u64(), RMP_4K, VMPL_R | VMPL::Vmpl3 as u64);
    ...
    let vmin: u64 = VMPL::VmplMax as u64;
    ...
    let ret: u32 = rmpadjust(vmsa.as_u64(), RMP_4K, VMPL_VMSA | VMPL::Vmpl3 as u64);  
    if ret != 0 {
        return false;
    }
