...
call_spec_t* ctx_get_call_info(context_t *ctx, intptr_t sysno, intptr_t args[]) {
        if (sysno > ctx->spec->max_sysno || sysno < 0) {
                printf("invalid syscall number %ld, max %ld\n", sysno, ctx->spec->max_sysno);
                exit(EXIT_FAILURE);
        }
                                ...
                                for (int i = 0; i < strct.n_fields; i++) {
                                        if (strct.fields[i].field == path[0]) {
                                                ...
                                                next = (scsan_arg_info_t) {
                                                        .addr = now.addr + strct.fields[i].offset,
                                                        .id = path[0]
                                                };
                                                break;
                                        }
        ...
        case ARRAY_TYPE:
        {
                ...
                if (!is_total_size) {
                        for (int i = 0; i < n_elem; i++) {
                                ...
                                scsan_arg_stack_push(s, (scsan_arg_info_t){.addr = arg + total_size, .id = arr.elem});
                                total_size += arg_size(ctx, s, arg + total_size, arr.elem);
                                scsan_arg_stack_pop(s);
                        }
                }
                return total_size;
...
void* deep_copy_arg(context_t *ctx, scsan_arg_stack_t *s, void *src_arg, arg_id_type_t ty, void *dst_arg, size_t *size, bool syscall_entry);

void* deep_copy_arg_with_stack_update(context_t *ctx, scsan_arg_stack_t *s, void *src_arg, arg_id_type_t ty, void *dst_arg, size_t *size, bool syscall_entry) {
        scsan_arg_stack_push(s, (scsan_arg_info_t){.addr = src_arg, .id = ty});
        void *ret = deep_copy_arg(ctx, s, src_arg, ty, dst_arg, size, syscall_entry);
        scsan_arg_stack_pop(s);
        return ret;
}
...
void* deep_copy_arg(context_t *ctx, scsan_arg_stack_t *s, void *src_arg, arg_id_type_t ty, void *dst_arg, size_t *size, bool syscall_entry) {
        ...
        case ARRAY_TYPE:
        {
                ctx_debug_log(ctx, "this is array type\n");
                array_type_t arr = info->array;
                size_t cur_copied_size = 0;
                while (cur_copied_size < copied_size) {
                        // copy the element one by one.
                        size_t size = 0;
                        deep_copy_arg_with_stack_update(ctx, s, src_arg+cur_copied_size, arr.elem, dst_arg+cur_copied_size, &size, syscall_entry);
        ...
        case STRUCT_TYPE:
        {
                ctx_debug_log(ctx, "this is struct type\n");
                struct_type_t strct = info->strct;
        ...
        case POINTER_TYPE:
        {
                ctx_debug_log(ctx, "this is pointer type\n");
                pointer_type_t ptr = info->pointer;
                if (syscall_entry) {
                        *(void**)dst_arg = deep_copy_arg_with_stack_update(ctx, s, *(void**)src_arg, ptr.pointee, NULL, NULL, syscall_entry);
                } else {
                        if (ptr.dir != IN_DIR) {
                                deep_copy_arg_with_stack_update(ctx, s, *(void**)src_arg, ptr.pointee, *(void**)dst_arg, NULL, syscall_entry);
                        }
                }
                break;
        }
...
uintptr_t scsan_syscall(context_t *ctx, intptr_t sysno, intptr_t args[]) {
        ...
        for (int i = 0; i < call->n_arg; i++) {
                deep_copy_arg_with_stack_update(ctx, &s, &args[i], call->args[i], &new_args[i], NULL, true);
        }
        ret = syscall(sysno, new_args[0], new_args[1], new_args[2], new_args[3], new_args[4], new_args[5]);
        ...
        for (int i = 0; i < call->n_arg; i++) {
                deep_copy_arg_with_stack_update(ctx, &s, &new_args[i], call->args[i], &args[i], NULL, false);
        }
        ctx_free_copied_mem(ctx);
        scsan_arg_stack_free(&s);
        return ret;
