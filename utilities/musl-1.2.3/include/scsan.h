...
/* Save global context */
extern context_t scsan_ctx;
extern bool      scsan_ctx_init;
extern bool      handle_scsan_syscall;
extern bool      syscall_args_debug;

// uintptr_t scsan_syscall(context_t *ctx, intptr_t sysno, intptr_t args[]);
// uintptr_t scsan_syscall(intptr_t sysno, intptr_t args[]);
uintptr_t scsan_syscall(intptr_t sysno, intptr_t args[], int arglen);
uintptr_t scsan_syscall_va(intptr_t sysno, ...);

typedef void* alloc_func_t(size_t);
void init_spec(spec_t *s);
void init_predefined_spec(spec_t *s);
