...
// HARSH
DEFINE_PER_CPU(struct sev_es_runtime_data*, enclave_runtime_data);
...
/* Harsh - added custom functions for enclave GHCB*/
extern void snp_register_ghcb_early(unsigned long paddr);
extern void snp_register_ghcb_vmpl_early(unsigned long paddr);
typedef unsigned int u32;

noinstr struct ghcb* __enclave_sev_get_ghcb(struct ghcb_state *state)
{
	struct sev_es_runtime_data *data;
	struct ghcb *ghcb;
	...
	data = this_cpu_read(enclave_runtime_data);
	ghcb = &data->ghcb_page;
...
EXPORT_SYMBOL_GPL(__enclave_sev_get_ghcb);
...
static noinstr void __sev_put_ghcb(struct ghcb_state *state)
{
	...
	// HARSH - modification to make the VMGEXIT get the enclave GHCB
	/* Revert back to the enclave GHCB */
	data = this_cpu_read(enclave_runtime_data);
	/* Sanity check */
	if (!data) return;
...
noinstr void __enclave_sev_put_ghcb(struct ghcb_state *state)
{
	struct sev_es_runtime_data *data;
	struct ghcb *ghcb;
...
EXPORT_SYMBOL_GPL(__enclave_sev_put_ghcb);
...
// HARSH - removed static
void *snp_alloc_vmsa_page(void)
{
	struct page *p;
...
EXPORT_SYMBOL_GPL(snp_alloc_vmsa_page);
...
static void snp_register_per_cpu_ghcb(void)
{
	...
	snp_register_ghcb_vmpl_early(__pa(ghcb));
...
// HARSH
void __init enclave_alloc_runtime_data(int cpu)
{
	struct sev_es_runtime_data *data;
	...
	per_cpu(enclave_runtime_data, cpu) = data;
}
...
// HARSH
void __init enclave_init_ghcb(int cpu)
{
	struct sev_es_runtime_data *data;
	int err;

	data = per_cpu(enclave_runtime_data, cpu);
...
void __init sev_es_init_vc_handling(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		init_ghcb(cpu);
		printk("[HARSH] allocated runtime data and GHCB for OS\n");
		// HARSH
		printk("[HARSH] allocated runtime data and GHCB for Enclave\n");
		enclave_alloc_runtime_data(cpu);
		enclave_init_ghcb(cpu);
	}
