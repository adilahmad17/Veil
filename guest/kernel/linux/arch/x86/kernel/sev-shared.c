...
int __svsm_msr_protocol(struct svsm_caa *caa, u64 rax, u64 rcx, u64 rdx, u64 r8, u64 r9)
{
	u64 val, resp;
	u8 pending;
	int ret;
...
EXPORT_SYMBOL_GPL(__svsm_msr_protocol);
...
void snp_register_ghcb_vmpl_early(unsigned long paddr)
{
	unsigned long pfn = paddr >> PAGE_SHIFT;
	u64 val;

	sev_es_wr_ghcb_msr(GHCB_MSR_REG_GPA_VMPL_REQ_VAL(pfn));
	VMGEXIT();
...
/* HARSH - made non static, non inline and exported */
void vc_ghcb_invalidate(struct ghcb *ghcb)
{
	ghcb->save.sw_exit_code = 0;
	__builtin_memset(ghcb->save.valid_bitmap, 0, sizeof(ghcb->save.valid_bitmap));
}
EXPORT_SYMBOL_GPL(vc_ghcb_invalidate);
