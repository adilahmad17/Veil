...
void audit_log_end(struct audit_buffer *ab)
{
	...
	/* Veil: If logging service is enabled, send the log
	 * to the backend veil logger module for protection. */
	if (is_logging_service_enabled()) {
		skb = ab->skb;
		nlh = nlmsg_hdr(skb);
		invoke_logging_service_protection((unsigned long) nlmsg_data(nlh), 
