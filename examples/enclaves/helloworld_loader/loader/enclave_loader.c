/*
* Loader functions borrowed from
...
static void load_obj(const char *name)
{
    ...
    if (fstat(fd, &sb)) {
        perror("Failed to get object info");
        exit(errno);
    }
...
static void *lookup_ext_function(const char *name)
{
    ...
    if (!strncmp(name, "puts", strlen("puts")))
        return puts;

    
    if (!strncmp(name, "printf", strlen("printf")))
        return printf;

	if (!strncmp(name, "exit_enclave", strlen("exit_enclave")))
        return exit_enclave;
...
static void parse_obj(void)
{
    ...
    unsigned long size = page_align(text_hdr->sh_size) + \
                                   page_align(data_hdr->sh_size) + \
                                   page_align(rodata_hdr->sh_size) + \
                                   page_align(sizeof(struct ext_jump) * num_ext_symbols);
    text_runtime_base = mmap(NULL, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
static void* get_entry_func(void)
{
    void *entry_address;
    entry_address = lookup_function("enclave_entry");
    if (!entry_address) {
        fputs("Failed to find function\n", stderr);
        exit(ENOENT);
    }
	return entry_address;
}

void load_enclave(const char *name, unsigned long *enclave_entry_address)
{
    load_obj(name);
    parse_obj();
    *enclave_entry_address = lookup_function("enclave_entry");
    printf("Enclave entry address: %lx\n", *enclave_entry_address);
    if (!enclave_entry_address) {
        fputs("Failed to find function\n", stderr);
    }
    printf("[*] Enclave object loaded successfully.\n");
}