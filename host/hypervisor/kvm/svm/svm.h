...
struct vcpu_svm {
	...
	// HARSH
	u64 ghcb_registered_gpa[SVM_SEV_VMPL_MAX][MAX_ENCLAVE];
	bool ghcb_in_use;
...
// HARSH - updated for multiple registered GHCBs
static inline bool ghcb_gpa_is_registered(struct vcpu_svm *svm, u64 val)
{
	int i;
	for(i=0; i<MAX_ENCLAVE; ++i){
		if (svm->ghcb_registered_gpa[svm->snp_current_vmpl][i] == val)
				return 1;
		}
	return 0;
}
