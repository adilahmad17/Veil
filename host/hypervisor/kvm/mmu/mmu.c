			     ...
			     kvm_pfn_t *pfn)
{
	...
	int r;
#if 0
	/* SCATTACK: Checking if the victim page fault reaches here. */
	if (scattack_initialized) {
		SCATTACK_PRINT("[*] Direct page fault.. (gva = %llx, hva = %lx) \n", 
				(gfn << PAGE_SHIFT), hva);
		if (hva != 0 && is_victim_page(hva)) {
			SCATTACK_PRINT("[*] Victim HVA faulted... \n");
		}
	}
#endif
				...
				u64 fault_address, char *insn, int insn_len)
{
	...
	/* SCATTACK: Checking if the victim page fault reaches here. */
	if (scattack_initialized) {
		if (fault->hva != 0 && is_victim_page(fault->hva)) {
			SCATTACK_PRINT("[*] Direct page fault.. (gva = %llx, hva = %llx) \n", 
